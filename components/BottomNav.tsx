...
const tabs = [{ href:'/', label:'Base', icon:Home }, { href:'/raid', label:'Saqueo', icon:Swords }]
export function BottomNav(){
  const path = usePathname()
  return (<nav className="fixed bottom-0 left-0 right-0 z-50">
    <div className="mx-auto max-w-md rounded-t-2xl bg-surface/90 backdrop-blur border-t border-white/5 shadow-soft">
      <ul className="grid grid-cols-2">
        {tabs.map(({href,label,icon:Icon})=>{
          ...
          return (<li key={href} className="relative">
