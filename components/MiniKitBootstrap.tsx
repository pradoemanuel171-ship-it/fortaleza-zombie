...
export default function MiniKitBootstrap({ loc }: { loc: Locale }){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        // Install MiniKit and immediately run auth in the same tick to avoid race (per docs FAQ)
        MiniKit.install()
        // Quick check
        ...
        if (finalPayload.status==='error'){
          setStage('error'); setErr((finalPayload as any)?.error || (finalPayload as any)?.reason || 'auth error'); return
        }
        ...
        if (!r.ok){ const d=await r.json().catch(()=>({})); setErr(d?.error||'verify failed'); setStage('error'); return }
        setStage('ok')
  ...
  if (stage==='blocked'){
    return <div className="fixed inset-0 bg-bg text-text flex items-center justify-center p-6"><div className="max-w-sm text-center space-y-2"><div className="text-lg font-semibold">{tClient(loc,'app.blocked')}</div></div></div>
  }
  // installing/auth/error -> show overlay
  return (
