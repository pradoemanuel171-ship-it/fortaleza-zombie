...
export default function RaidClient({ loc }: { loc: Locale }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {state==='ready' && rival && (
        <div className="space-y-4">
          <div className="rounded-2xl bg-surface p-4 shadow-soft anim-slide-up">
      ...
      {state==='attacking' && (
        <div className="rounded-2xl bg-surface p-6 anim-pop">
          <CirclePerfect onDone={onAttackResult} />
        </div>
      )}

      {state==='result' && (
        <div className="rounded-2xl bg-surface p-4 space-y-3 anim-pop">
          <div>{message}</div>
